...
// Hotel-style nights array (checkIn inclusive, checkOut exclusive)
function getNights(checkIn: Date, checkOut: Date): Date[] {
  const dates: Date[] = [];
  const curr = new Date(checkIn);

  while (curr < checkOut) {
    dates.push(new Date(curr));
    curr.setUTCDate(curr.getUTCDate() + 1); // avoid timezone issues
  }
...
export async function POST(req: Request) {
  try {
    ...
    // 2) Read request body
    const body = await req.json();
    const { hotelId, checkIn, checkOut, rooms = 1, guests = 1 } = body;
    ...
    if (guests > hotel.maxGuests) {
      return NextResponse.json(
        {
          success: false,
          message: `Max guests for this hotel is ${hotel.maxGuests}`,
        },
        { status: 400 }
      );
    }
    ...
    const finalPrice = hotel.discountedPrice || hotel.price;
    const totalPrice = nights.length * finalPrice * rooms;

    // 5) Transaction: check + deduct inventory + create booking
    const booking = await prisma.$transaction(async (tx) => {
