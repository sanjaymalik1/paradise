...
export default function Search() {
  ...
  const filteredHotels = useMemo(() => {
    return hotels.filter((hotel: Hotel) => {
      const priceMatch = filters.price.max >= hotel.discountedPrice && filters.price.min <= hotel.discountedPrice;
  ...
  return (
    <div className="h-dvh ">
      ...
      <div className="flex w-full max-h-screen overflow-hidden pt-18">
        ...
        {/* {
          hotels.length === 0 ? (
            <p className="p-6">No hotels available for these details.</p>
        ) : (
